...
export default function ThreatMap() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Submodule imports — tree-shakes out unused d3.
      // d3-transition is imported for its side effect (adds .transition() to selections).
      const [{ geoEquirectangular }, { select }, { easeCubicInOut }] = await Promise.all([
        import("d3-geo"),
        import("d3-selection"),
        import("d3-ease"),
      ...
      // Pre-computed land dots (generated by scripts/generate-dots.mjs)
      try {
        const base = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
        const dots: [number, number][] = await fetch(`${base}/land-dots.json`).then((r) => r.json());
        if (!mounted) return;
      ...
      // Pause animations when the map scrolls out of view
      ...
      const spawnPulse = () => {
        if (!mounted || !isVisible || !inView) return;
      ...
      const spawnArc = () => {
        ...
        path
          ...
          .transition().duration(1400).ease(easeCubicInOut)
    ...
    // Defer init until the browser is idle so we don't fight the main thread on load
    const w = window as IdleWindow;
    if (w.requestIdleCallback) {
      idleHandle = w.requestIdleCallback(init, { timeout: 2000 });
    } else {
